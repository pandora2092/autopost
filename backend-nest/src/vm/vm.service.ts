...
export class VmService {
  ...
  findAll() {
    ...
    const rows = db.prepare(
      `SELECT v.id, v.name, v.libvirt_domain, v.mac, v.proxy_id, v.adb_address, v.android_id, v.status, v.instagram_installed, v.youtube_installed, v.vk_installed, v.created_at,
  ...
  installVk(id: string, apkPath?: string) {
    const vm = this.db.getDb()
      .prepare('SELECT id, adb_address, libvirt_domain FROM vm WHERE id = ?')
      .get(id) as { id: string; adb_address: string | null; libvirt_domain: string } | undefined;
    if (!vm) throw new NotFoundException('VM не найдена');
    ...
    const { output } = this.vmManager.installVk(adbAddress, apkPath);
    this.db.getDb().prepare('UPDATE vm SET vk_installed = 1 WHERE id = ?').run(id);
    return { ok: true, adb_address: adbAddress, output };
  }
